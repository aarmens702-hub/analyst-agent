...
- **Aarmen hand-writes the core:** the agent loop, the skill lifecycle/governance, the provenance DAG, and the prompts. For anything under `src/analyst_agent/` touching those areas: propose diffs and explain tradeoffs — do not rewrite unasked.
...
- Run: `uv run python -m analyst_agent`
...
Full spec: `~/Desktop/mailo/coop-project/analyst-agent-brief.md` and `analyst-agent-build-research.md` (stack patterns, gotchas, disease taxonomy, datasets).
