...
- R15. Images travel b64 inline; the client decodes to `workspace/<session>/artifacts/cell_<request_id>_<i>.png` and hands paths upward. (Amended 2026-08-11: the request id replaces `exec_count`, which is only known once the result arrives and would block live streaming.)
